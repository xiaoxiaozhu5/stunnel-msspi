...
#-----------------------------------------------------------------------------
# Соединяется с машиной из пула (её сервером: см. StartServer())
sub ConnectToThisServer($$) 
{
    my $datalength;
    my $difftime;
	...
	#rtLog('Client get it: '.$r_data.' and send back.');
        if ($correct_start and $correct_finish) { 
            ...
            # разрыв соединения. попробуем подключиться еще раз          
        }                                                    
...
#-----------------------------------------------------------------------------
# TODO: плохая функция, нарушающая инкапсуляцию (много знает про сертификат из stunnel.pl)
